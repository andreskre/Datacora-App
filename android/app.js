...
const state = {
  ...
  assignRbdSearch: "",
  assignSelectedRbd: "",
  assignType: "Plan Preventivo Mantención",
  assignPriority: "Media",
  assignDueAt: "",
  assignDescription: "",
...
function mapSupabaseTask(row) {
  ...
  return {
    ...
    dueAt: formatDateLabel(row.due_date),
...
async function fetchProcedures() {
  ...
  const rows = await apiRequest("/api/procedures");
...
async function refreshProcedures(options = {}) {
  ...
  try {
    ...
    if (manual) setState({ procedureError: error.message || "No se pudieron cargar los procedimientos." });
  } finally {
...
function establishmentsForUser(user = loggedUser()) {
  if (isAdmin(user)) return [...establishments].sort((left, right) => Number(left.rbd) - Number(right.rbd));
  const branches = userBranches(user);
  return establishments
...
function currentFormProgressSnapshot() {
  return {
    ...
    infrastructureDraft: state.infrastructureDraft,
...
function formProgressStateForTask(taskId) {
  ...
  return {
    ...
    infrastructureDraft: { ...emptyInfrastructureDraft(), ...(progress.infrastructureDraft ?? {}) },
...
function photoAttachmentsForRecord(photoRows, sectionCode, record, index) {
  ...
  if (exactMatch) {
    ...
    return exactMatches.slice(0, MAX_EVIDENCE_PHOTOS_PER_ELEMENT);
  }
...
async function ensureRemoteSubmissionRecords(task, options = {}) {
  ...
  const preserveLocalEvidence = async (sectionCode, remoteRecords = [], localRecords = []) => {
    ...
    return Promise.all(sortedRemote.map(async (remoteRecord, index) => {
      ...
      const evidencePhotos = uniqueEvidencePhotos([
        ...remoteRecordPhotos,
        ...localPhotos,
        ...remotePhotos
      ])
...
function tasksScreen() {
  const filters = ["Pendientes", "Atrasadas", "Completadas"];
  const currentFilter = filters.includes(state.filter) ? state.filter : "Pendientes";
  const user = loggedUser();
  const search = normalizeSearch(state.technicianTaskSearch || "");
  ...
  const visibleTasks = assignedTasks.filter((task) => {
    if (currentFilter === "Pendientes") return !taskIsCompleted(task) && !taskIsOverdue(task);
    if (currentFilter === "Atrasadas") return taskIsOverdue(task);
  ...
  return `
    ...
    <main class="screen with-nav">
      ${permissionCheckCard()}
      <div class="segmented">
        ${filters.map((filter) => `<button class="${currentFilter === filter ? "active" : ""}" data-action="filter" data-filter="${filter}">${filter}</button>`).join("")}
      </div>
      <section class="task-search-card">
        <label class="field compact-field">
          <span>Buscar tarea</span>
          <div class="field-control">${icons.info}<input type="search" data-role="technician-task-search" value="${escapeAttribute(state.technicianTaskSearch || "")}" placeholder="RBD o establecimiento..." autocomplete="off" /></div>
        </label>
      </section>
      <div class="task-refresh-bar">
        <button type="button" class="task-refresh-button" data-action="refresh-tasks" ${state.taskRefreshUiBusy ? "disabled" : ""}>
          ${icons.sync}
          <span>${state.taskRefreshUiBusy ? "Actualizando..." : "Actualizar"}</span>
        </button>
      </div>
      <section class="stack">
        ${visibleTasks.length ? visibleTasks.map(taskCard).join("") : emptyState("Sin tareas", "No hay tareas para este filtro.")}
      </section>
    </main>
    ${bottomNav()}
  `;
...
function bitacoraRecordExtraRows(record) {
  ...
  const rows = Object.entries(record)
    ...
    .map(([key, value]) => [
      fieldLabel(key),
      canonicalFieldKey(key) === "installedArticles" ? installedArticlesAnswerValue(value) : typeof value === "object" ? JSON.stringify(value) : String(value)
    ])
    .filter(([, value]) => value !== "");
  if (!rows.length) return "";
  return `
    ...
    <article class="jm-bitacora-record">
      ${hasPhoto ? `
        <div
          ...
          style="display:grid;grid-template-columns:repeat(${visiblePhotos.length},minmax(0,1fr));gap:10px;width:100%;">
...
function taskIsEmergency(task) {
  const value = `${task.type ?? ""} ${task.priority ?? ""} ${task.status ?? ""}`.toLowerCase();
  return value.includes("emergencia") || value.includes("urgente") || value.includes("alta");
}
...
function jmBranchMetrics(branchName) {
  ...
  const overdue = branchTasks.filter(taskIsOverdue);
  const pending = branchTasks.filter((task) => !taskIsCompleted(task) && !taskIsOverdue(task));
  const emergencies = branchTasks.filter((task) => !taskIsCompleted(task) && taskIsEmergency(task));
  const completed = branchTasks.filter(taskIsCompleted);
...
function jmDashboardDetails(user, branchTasks, activeTechnicians, panel) {
  const overdue = branchTasks.filter(taskIsOverdue);
  const pending = branchTasks.filter((task) => !taskIsCompleted(task) && !taskIsOverdue(task));
  const urgent = branchTasks.filter((task) => !taskIsCompleted(task) && taskIsEmergency(task));
  const completed = branchTasks.filter(taskIsCompleted);
  ...
  const selectedTasks = ["urgent", "overdue", "completed", "pending"].includes(panel)
    ? applyJmCompletedFilters(baseTasks, panel)
    : baseTasks;
    ...
    <main class="screen with-nav jm-screen jm-tasks-screen">
      <section class="jm-panel">
        ...
        <div class="action-menu jm-actions">
          ...
          ${actionTile("Carga masiva", "Subir una planilla Excel con múltiples visitas.", "bulk-assign", icons.clipboard)}
        </div>
      </section>
    </main>
    ${bottomNav()}
...
function adminAssignScreen() {
  ...
  const taskTypes = Object.values(formBlueprints).map((blueprint) => blueprint.taskType);
  const selectedTechnicianEmail = technicians.some((user) => user.usuario === state.assignTechnician)
    ? state.assignTechnician
    : "";
  const selectedTechnician = technicians.find((user) => user.usuario === selectedTechnicianEmail);
  const branchEstablishments = selectedBranch ? establishmentsByBranch(selectedBranch) : [];
  ...
  return `
    ...
    <main class="screen with-nav ${isLimitedManagerUser(actor) ? "jm-assign-screen" : ""}">
      <form onsubmit="return validateAssignTaskForm(event)" class="create-user-form ${isLimitedManagerUser(actor) ? "jm-assign-form" : ""}" data-role="assign-task-form">
        <h3>${isLimitedManagerUser(actor) ? "Datos de la tarea" : "Nueva tarea"}</h3>
        <label class="field compact-field"><span>Zona</span><div class="field-control">${icons.branch}<select name="branch" data-role="assign-branch" ${branchOptions.length <= 1 ? "disabled" : ""}>${branchOptions.map((branch) => `<option value="${escapeAttribute(branch)}" ${branch === selectedBranch ? "selected" : ""}>${escapeHtml(branch)}</option>`).join("")}</select>${branchOptions.length <= 1 ? `<input type="hidden" name="branch" value="${escapeAttribute(selectedBranch)}" />` : ""}</div></label>
        <label class="field compact-field"><span>Técnico</span><div class="field-control"><select name="technician" data-role="assign-technician">${technicians.map((user) => `<option value="${user.usuario}" ${user.usuario === selectedTechnicianEmail ? "selected" : ""}>${user.nombre} · ${user.sucursal}</option>`).join("")}</select></div></label>
        <div class="form-grid">
          <label class="field compact-field"><span>Tipo</span><div class="field-control"><select name="type" data-role="assign-type">${taskTypes.map((type) => `<option value="${type}" ${type === state.assignType ? "selected" : ""}>${type}</option>`).join("")}</select></div></label>
          <label class="field compact-field"><span>Prioridad</span><div class="field-control"><select name="priority" data-role="assign-priority">${["Media", "Alta", "Baja"].map((priority) => `<option value="${priority}" ${priority === state.assignPriority ? "selected" : ""}>${priority}</option>`).join("")}</select></div></label>
        </div>
        <label class="field compact-field searchable-combo"><span>Establecimiento / RBD</span>
          <div class="field-control">${icons.info}<input name="establishmentSearch" type="search" data-role="assign-establishment-search" value="${escapeAttribute(searchValue)}" placeholder="Escribe RBD o nombre del establecimiento..." autocomplete="off" ${selectedTechnician ? "" : "disabled"} /></div>
        ...
        <div class="assignment-zone-note">
          ${selectedBranch ? `${icons.branch}<span>Zona seleccionada: <strong>${selectedBranch}</strong>. Mostrando <strong>${filteredEstablishments.length}</strong> de ${branchEstablishments.length} RBD asociados.</span>` : `${icons.alert}<span>No hay zonas disponibles para este perfil.</span>`}
        </div>
        <label class="field compact-field"><span>Fecha planificada</span><div class="field-control"><input name="dueAt" type="date" data-role="assign-due-at" value="${escapeAttribute(state.assignDueAt)}" min="${today}" /></div></label>
        ...
        <button class="button primary" type="button" data-action="assign-task">${isLimitedManagerUser(actor) ? "Continuar" : "Asignar tarea"}</button>
      </form>
      ${state.actionMessage ? `<div class="temp-password">${state.actionMessage}</div>` : ""}
    </main>
    ${bottomNav()}
...
async function assignTask() {
  ...
  const type = state.assignType || String(formData.get("type") ?? "Plan Preventivo Mantención");
  ...
  if (!technician || !selectedEstablishment) {
    setState({ actionMessage: "Selecciona técnico y establecimiento disponible para su sucursal." });
    return;
  }
  ...
  const newTask = {
    ...
    description: state.assignDescription.trim() || String(formData.get("description") ?? "").trim() || "Tarea asignada por administrador.",
    assignedBy: loggedUser().nombre,
    assignedTo: technician.usuario,
    assignedAt: todayDateLabel(),
    dueAt,
    dueDateIso,
    status: state.assignPriority === "Alta" ? "Urgente" : "Pendiente",
    priority: state.assignPriority,
    syncStatus: "synced",
    form: { blueprintKey, requiredSections, criticalSections, sectionMinimums }
  };
  ...
  if (hasSupabaseConfig()) {
    try {
      ...
      showErrorToast(`No se pudo crear la tarea en ${hasSqlServerApiConfig() ? "SQL Server" : "Supabase"}`, error.message);
      return;
    }
  }
  ...
  showSuccessToast("tarea asignada correctamente", `Asignada a ${technician.nombre}.`);
}
...
function isInfrastructureDraftComplete(draft = currentInfrastructureDraftFromForm()) {
  return isSimpleRecordDraftComplete(draft) && Boolean(draft.achsSignage);
}
...
function validateSimpleRecordDraft(draft, section) {
  ...
  if (!base.valid) return base;
  if (section === "infrastructure" && !draft.achsSignage) return validationResult("Falta responder: Señalética ACHS.", "infrastructure-achs-signage");
...
function updateInfrastructureDraft(nextDraft, options = {}) {
  updateSectionDraft("infrastructureDraft", "infrastructureError", nextDraft, options);
}
...
async function attemptLogin() {
  ...
  if (hasSupabaseConfig()) {
    try {
      ...
      try {
        const [supabaseTasks, supabaseIncidents] = await Promise.all([
...
function render() {
  ...
  rootEl.innerHTML = (screens[state.route] ?? screens[fallbackRoute])() + imagePreviewOverlay() + articlePickerOverlay() + successToast() + gpsDebugFloatingPanel();
...
rootEl.addEventListener("click", async (event) => {
  ...
  if (action === "filter") setState({ filter: element.dataset.filter });
  ...
  if (action === "select-establishment") {
    ...
    const selected = establishmentsByBranch(selectedBranch).find((item) => item.rbd === element.dataset.rbd);
    if (!selected) {
      setState({ ...formState, assignSelectedRbd: "", assignRbdSearch: "", actionMessage: "El RBD seleccionado no pertenece a la zona activa." });
      return;
    }
    setState({
      ...formState,
...
rootEl.addEventListener("input", (event) => {
  ...
  if (element?.matches('[data-role="technician-task-search"]')) {
    const value = element.value;
    setState({ technicianTaskSearch: value });
    const nextInput = rootEl.querySelector('[data-role="technician-task-search"]');
    nextInput?.focus({ preventScroll: true });
    nextInput?.setSelectionRange(value.length, value.length);
    return;
  }
...
rootEl.addEventListener("change", (event) => {
  ...
  if (element?.matches('[data-role="assign-technician"]')) {
    setState({ ...currentAssignFormState(), assignTechnician: element.value, assignRbdSearch: "", assignSelectedRbd: "", actionMessage: "" });
  }
...
rootEl.addEventListener("input", (event) => {
  ...
  if (element?.matches('[data-role="assign-description"]')) {
    setState({ ...currentAssignFormState(), assignDescription: element.value, actionMessage: "" });
    const nextTextarea = rootEl.querySelector('[data-role="assign-description"]');
    nextTextarea?.focus();
    nextTextarea?.setSelectionRange(element.value.length, element.value.length);
  }
